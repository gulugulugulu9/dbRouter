...
public class DBRouterJoinPoint {
    ...
    public Object doRouter(ProceedingJoinPoint jp, DBRouter dbRouter) throws Throwable {
        ...
        // 路由属性, uid:100001, 读取100001

        String dbKeyAttr = getAttrValue(dbKey, jp.getArgs());
        // 路由策略  算出分库和分表的后缀
        // 放行方法之前，  就先threadLocal中保存分库分表信息。
        // 后续先mybatis插件拦截sql语句分表，再去spring-datasorce中读取数据库
        dbRouterStrategy.doRouter(dbKeyAttr);
        // 返回结果
        try {
            return jp.proceed();
            // 函数放行， spring在选取数据源信息时候，读取我自己定义DynamicDataSource
            // 在mybatis 执行sql 语句时， 读取我的DynamicMybatisPlugin
        } finally {
